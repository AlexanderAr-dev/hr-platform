...
export default class AsyncModule extends ReactLoadableModuleAsync<{}> {
        ...
        renderer: {
          ...
          renderer: (props: TableCellRendererProps<TableDataset<UserItem>>) => (
            <div className={UserRowStyles}>
              ...
              <Button icon={<UnorderedListOutlined />} onClick={() => this.openUserModal(props.value!)} />
            </div>
          ),
        } as const,
        width: 240,
  ...
  @action handleUserModalSubmit = (updatedUser: UserItemEntity) => {
    ...
    this.parser.recalculateForbiddenIntersections(this.users);
    ...
    return (
      <div>
        ...
        <div className={ButtonContainerStyles}>
          <Button onClick={() => this.openUserModal(undefined)}>Добавить нового сотрудника</Button>
        </div>
        <div className={UploadContainerStyles}>
          <FileUploader
            files={[]}
            multiple={false}
