...
interface MultipleDatePickerInterface {
  ...
  disabledDate?: (currentDate: DateTime | null) => boolean;
}
...
function MultipleDatePicker({
  ...
  const disablePastDates = (currentDate: DateTime | null) => {
    if (!currentDate) return false;
    const today = DateTime.local().startOf("day");
    return currentDate.startOf("day") < today;
  };
    ...
    <ConfigProvider locale={locale}>
      <div className={cn(className, wrapperStyles)}>
        <LuxonDatePicker.RangePicker
          ...
          disabledDate={disabledDate ?? disablePastDates}
        />
        {errorMessage && (
